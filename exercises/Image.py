...
W, H = pt(inch(9)), pt(inch(12)) # Document size, It's in pt, if not, display error message.
...
#image note style
text2 = """LEFT AND ABOVE:
Experimental design for RoAndCo for Sight Unseen OFF SITE 2015 """
font2 = findFont('Roboto-Regular')
style2 = dict(font=font2, fontSize=9, leading=em(1.5), textFill=0.3, hyphenation=True)
...
parah = 450
newLine(x=225, y=375, w=1, h=parah, stroke=(0, 0, 0,0.5), strokeWidth=0.5, parent=page)
newLine(x=422, y=375, w=1, h=parah, stroke=(0, 0, 0,0.5), strokeWidth=0.5, parent=page)

# Display Image
img = newImage('Img2.jpg', padding=0, parent=page, w=PW, conditions=(Top2Top(), Fit2Width(), SolveBlock(), Shrink2BlockBottom()), yAlign=BOTTOM, fill=color(0, 1, 0, 0.3), stroke=color(1, 0, 0))

# Text box for the image note
newTextBox(t2, w=170, h=100, parent=page, xAlign=CENTER, conditions=(Left2Left(), Middle2Middle()))
...
c1 = newTextBox(t, w=CW, h=parah, name='c1', parent=page, xAlign=CENTER, nextElementName='c2', conditions=[Center2Center(), Bottom2Bottom(), Overflow2Next()])
# Text without initial content, will be filled by overflow of c1.
c2 = newTextBox(w=CW, h=parah, name='c2', parent=page, conditions=[Right2Right(), Bottom2Bottom()])
# Solve the page/element conditions
doc.solve()
# Export the document.
doc.export('_export/Image.jpg')
