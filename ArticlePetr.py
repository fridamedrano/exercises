...
newRect(w=W/5, h=H, stroke=noColor, parent=page, xAlign=CENTER, conditions=(Right2Right()), fill=(253/255,239/255,239/255))
...
# Petr: You could try to make colums wider by gutter/2 (G/2) and add padding to the right
# c1.pr = G/2, then you can add a border on the right side of the text column
# This way the height of the line is automatic connected to height and position of the text columns.

# Text Box for the Quote style
c0 = newTextBox(t2, w=W/4, h=H/3, z=10, parent=page, xAlign=CENTER, conditions=(Right2Right(), Middle2Middle()))
# Lines for Quote 
linew2= PW/10
linep= w=PW-125
...
c1 = newTextBox(t, pl=G/2, pr=G/2, w=CW, name='c1', parent=page, nextElementName='c2', 
	conditions=[Left2Left(), Top2Top(), Fit2Height(), Overflow2Next()])
# Text without initial content, will be filled by overflow of c1.
c2 = newTextBox(pl=G/2, pr=G/2, w=CW, name='c2', parent=page, 
	conditions=[Left2Col(2), Top2Top(), Fit2Height()])

c1.borderLeft = c1.getBorderDict(stroke=color(0, 0, 0,0.5), strokeWidth=3)
c1.borderRight = c1.getBorderDict(stroke=color(0, 0, 0,0.5), strokeWidth=3)
c2.borderRight = c2.getBorderDict(stroke=color(0, 0, 0,0.5), strokeWidth=3)

# Solve the page/element conditions
doc.solve()

# Export the document to this PDF file.
doc.export('_export/ArticlePetr.jpg')
